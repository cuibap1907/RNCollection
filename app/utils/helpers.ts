...
export const selectPhoto = (callback, onlyTake = true, maxPick = 10) => {
...
export const requestAndroidCameraPermission = async () => {
  if (Platform.OS === "android" && Platform.Version > 23) {
    try {
      ...
      return { success: false, error: translate("unknowError") }
    }
  } else {
    return { success: true, error: null }
  }
...
export const imagePathOf = (host, fileName) =>
  isEmpty(fileName) ? "" : `${host}storages/${fileName}`
